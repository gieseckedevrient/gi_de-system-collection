...
$spec = @{
  options = @{
    name = @{ type = "str" ; choices = "all", "SampleVideos", "InternetCache", "AccountPictures", "SavedGames", "ComputerFolder", "ConflictFolder",
    ...
    "HomeGroup", "Documents", "PublicMusic", "ImplicitAppShortcuts", "CommonAdminTools", "OriginalImages", "UserProgramFilesCommon" ; default = "all" }
  }
...
#fetch packages
try {
      $Packages = Get-KnownFolderPath -Name $name
  }
  catch {
    $module.FailJson("Error fetching KnownFolders $($_.Exception.Message)")
  }

  #deal with results
  try {
    if(!$Packages) {
      $module.Result.msg = "KnownFolder is not known"
      $module.Result.exists = $false
      $module.Result.FolderPaths = $null
    } else {
      $module.Result.msg = "KnownFolder found"
      $module.Result.exists = $true
      $module.Result.FolderPaths = $Packages
    }
  }
  catch {
